"""
    Алгоритм Дейкстры - нахождение кратчайшего пути между узлами графа
...
class Graph:
    ...
    # Внимание! Ряд - источник, а столбец - назначение
    ...
    # убирает связь в направленной манере (node1 к node2)
    ...
    # получает вес, представленный перемещением от node1 к node2.
...
if __name__ == '__main__':
    ...
    w_graph = Graph.create_from_nodes([a, b, c, d, e, f, g, h, i])

    w_graph.connect(a, b, 15)
    w_graph.connect(a, c, 20)
    w_graph.connect(a, e, 12)
    w_graph.connect(b, c, 12)
    w_graph.connect(b, d, 14)
    w_graph.connect(c, d, 17)
    w_graph.connect(c, f, 21)
    w_graph.connect(d, e, 22)
    w_graph.connect(d, f, 5)
    w_graph.connect(e, g, 2)
    w_graph.connect(g, c, 19)
    w_graph.connect(d, h, 16)
    w_graph.connect(e, i, 19)
    w_graph.connect(h, i, 8)
    w_graph.connect(i, d, 5)
